<!doctype html>
<html>
<head>
<#include "/template/sn_head.ftl">
	<!--[if lt IE 10]> 
	<style>
		.head-bottom{background:url(/html/sn-static/static/image/navbanner.png) no-repeat;}
	</style>
	<![endif]-->
</head>

<body>
	<#include "/template/sn_header.ftl">
    <div class="wrapper" id="wrapper">
    	<div class="container">
        	<div class="row mgt15">
		<@channelDirective channelId=26>
            	<h1 class="set_font"><a href="${channel.url!}" >${channel.displayName!}</a></h1>
            	<div class="col-md-4 col-lg-4">
                	<div class="dataSubjt-left port-1 effect-2">
			<!--<a href="./zt/cqfw/index.htm" >
                    		<img src="/html/sn-static/static/image/sjzt1.png" width="100%" height="162">	
			</a>-->

			    <div class="image-box">
				<img src="/html/sn-static/static/image/sjzt1.png" width="100%" height="162" alt="">
			    </div>
			    <div class="text-desc">
				<a href="${channel.url!}" class="btn"><h3>${channel.displayName!}</h3></a>
				<p>${channel.desc!}</p>
				
			    </div>
		                
                    </div>
                </div>
		</@channelDirective>
                <div class="col-md-8 col-lg-8">
                	<div class="dataSubjt-right">
                    	<ul class="dataSubjt-right-top clearfix">
                        	<li>
                            	<div class="panel-body">
                                    <p>行政村总数</p>
                                    <div class="lineH30">
                                        <i class="ico1">&nbsp;</i>
                                        <span class="num" id="num1">3941</span>
                                        <span>个</span>
                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>农村地区常住户数</p>
                                    <div class="lineH30">
                                        <i class="ico2">&nbsp;</i>
                                        <span class="num" id="num2">232.6</span>
                                        <span>万户</span>
                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>农村地区常住人口</p>
                                    <div class="lineH30">
                                        <i class="ico3">&nbsp;</i>
                                        <span class="num" id="num3">612.6</span>
                                        <span>万人</span>
                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>农业户籍人口</p>
                                    <div class="lineH30">
                                        <i class="ico4">&nbsp;</i>
                                        <span class="num" id="num4">310.7</span>
                                        <span>万人</span>
                                    </div>
                                </div>
                            <li>
			    <li>
                            	<div class="panel-body">
                                    <p>农业户籍家庭</p>
                                    <div class="lineH30">
                                        <i class="ico5">&nbsp;</i>
                                        <span class="num" id="num4">100.7</span>
                                        <span>万户</span>
                                    </div>
                                </div>
                            <li>
			    <li>
                            	<div class="panel-body">
                                    <p>农村劳动力</p>
                                    <div class="lineH30">
                                        <i class="ico6">&nbsp;</i>
                                        <span class="num" id="num4">181.8</span>
                                        <span>万人</span>
                                    </div>
                                </div>
                            <li>
                            
                        </ul>
                    </div>
                </div>
            </div>
            <div class="border-gray mgt15"></div>
            <div class="row mgt15">
            	<@channelDirective channelId=28>
            	<h1 class="set_font"><a href="${channel.url!}" >${channel.displayName!}</a></h1>
            	<div class="col-md-4 col-lg-4">
                	<div class="dataSubjt-left port-1 effect-2">
			<!--<a href="./zt/xxny/index.htm" >
                    		<img src="/html/sn-static/static/image/sjzt2.png" width="100%" height="162">	
			</a>-->

			    <div class="image-box">
				<img src="/html/sn-static/static/image/sjzt2.png" width="100%" height="162" alt="">
			    </div>
			    <div class="text-desc">
				<a href="${channel.url!}" class="btn"><h3>${channel.displayName!}</h3></a>
				<p>${channel.desc!}</p>
				
			    </div>
		                
                    </div>
                </div>
		</@channelDirective>
                <div class="col-md-8 col-lg-8">
                	<div class="dataSubjt-right">
                    	<ul class="dataSubjt-right-top clearfix">
                        	<li>
                            	<div class="panel-body">
                                    <p>民俗旅游园区</p>
                                    <div class="lineH30">
                                        <i class="ico21">&nbsp;</i>

                                        <span class="num" id="num021">704</span>
                                        <span>个</span>

                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>民俗旅游户</p>
                                    <div class="lineH30">
                                        <i class="ico22">&nbsp;</i>

                                        <span class="num" id="num022">3103</span>
                                        <span>户</span>

                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>民俗旅游村</p>
                                    <div class="lineH30">
                                        <i class="ico23">&nbsp;</i>

                                        <span class="num" id="num023">335</span>
                                        <span>个</span>

                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>特色农产品</p>
                                    <div class="lineH30">
                                        <i class="ico24">&nbsp;</i>

                                        <span class="num" id="num024">101</span>
                                        <span>种</span>

                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>星级园区</p>
                                    <div class="lineH30">
                                        <i class="ico25">&nbsp;</i>

                                        <span class="num" id="num025">79</span>
                                        <span>家</span>

                                    </div>
                                </div>
                            <li>
			    <li>
                            	<div class="panel-body">
                                    <p>美丽乡村</p>
                                    <div class="lineH30">
                                        <i class="ico26">&nbsp;</i>

                                        <span class="num" id="num025">93</span>
                                        <span>个</span>

                                    </div>
                                </div>
                            <li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="border-gray mgt15"></div>
            <div class="row mgt15" style="margin-bottom:15px;">
            	<@channelDirective channelId=29>
            	<h1 class="set_font"><a href="${channel.url!}" >${channel.displayName!}</a></h1>
            	<div class="col-md-4 col-lg-4">
                	<div class="dataSubjt-left port-1 effect-2">
			<!--<a href="./zt/tjnj/index.htm" >
                    		<img src="/html/sn-static/static/image/sjzt3.png" width="100%" height="162">	
			</a>-->

			    <div class="image-box">
				<img src="/html/sn-static/static/image/sjzt3.png" width="100%" height="162" alt="">
			    </div>
			    <div class="text-desc">
				<a href="${channel.url!}" class="btn"><h3>${channel.displayName!}</h3></a>
				<p>${channel.desc!}</p>
				
			    </div>
		                
                    </div>
                </div>
		</@channelDirective>
                <div class="col-md-8 col-lg-8">
                	<div class="dataSubjt-right">
                    	<ul class="dataSubjt-right-top clearfix">
                        	<li>
                            	<div class="panel-body">
                                    <p>资产总额</p>
                                    <div class="lineH30">
                                        <i class="ico31">&nbsp;</i>
                                        <span class="num" id="num021">6879.6</span>
                                        <span>亿元</span>
                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>集体经济总收入</p>
                                    <div class="lineH30">
                                        <i class="ico32">&nbsp;</i>
                                        <span class="num" id="num022">723.7</span>
                                        <span>亿元</span>
                                    </div>
                                </div>
                            <li>
			     <li>
                            	<div class="panel-body">
                                    <p>主营业务收入</p>
                                    <div class="lineH30">
                                        <i class="ico33">&nbsp;</i>
                                        <span class="num" id="num022">498.8</span>
                                        <span>亿元</span>
                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>主营业务成本</p>
                                    <div class="lineH30">
                                        <i class="ico34">&nbsp;</i>
                                        <span class="num" id="num023">217.8</span>
                                        <span>亿元</span>
                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>利润总额</p>
                                    <div class="lineH30">
                                        <i class="ico35">&nbsp;</i>

                                        <span class="num" id="num024">43.3</span>
                                        <span>亿元</span>

                                    </div>
                                </div>
                            <li>
                            <li>
                            	<div class="panel-body">
                                    <p>农用地面积</p>
                                    <div class="lineH30">
                                        <i class="ico36">&nbsp;</i>

                                        <span class="num" id="num025">375226.9498</span>
                                        <span>公顷</span>

                                    </div>
                                </div>
                            <li>
                            
                        </ul>
                    </div>
                </div>	
            </div>
          <!--<div class="row mgt15">
            	<div class="col-md-12">
                	<div class="more" "set_2_button set_2_btn-2 icon-down">
                    	<a href="/topiecms/portal/channel/hdjl/29_1.htm" target="_blank">查看更多专题</a>
                    </div>
                </div>
            </div>-->
        </div>
    </div>
    
	<#include "/template/sn_footer.ftl">
	<script type="text/javascript" src="/html/sn-static/static/js/webindex.js"></script>
    <!--右侧浮动框-->
    <div>
        <script language="JavaScript" type="text/javascript"> 
          /*  lastScrollY511=0; 
            function lastScrollY512(){  
                var diffY513; 
				var fh = 162;
				var tp=300;
				var scrollHeight= document.body.scrollHeight;
				var h=218;
                    if (document.documentElement && document.documentElement.scrollTop) 
                        diffY513 = document.documentElement.scrollTop; 
                    else if (document.body) 
                            diffY513 = document.body.scrollTop; 
                        else{}
                        percent5=(diffY513-lastScrollY511);
					  
                        if(percent5>0){
                            percent5=Math.ceil(percent5);  
							}
                        else 
                            percent5=Math.floor(percent5); 
							if(lastScrollY511+percent5 > scrollHeight-(fh+tp+h)){return;}
							 
                            document.getElementById("loveright").style.top=parseInt(document.getElementById ("loveright").style.top)+percent5+"px"; 
                            lastScrollY511=lastScrollY511+percent5;  
                            } 
                        suspendcode12="<div id=\"loveright\" style='z-index: 99;right:30px;position:absolute;TOP:300px;'><a href=\'\' target=\'_blank\'><img style='' border=0 src=static/image/abar.png></a></div>" 
                        document.write(suspendcode12);    
                        window.setInterval("lastScrollY512()",1); */
        </script>
        <script type="text/javascript">
			/*var oZt = document.getElementById("zt");
			var header = document.getElementById("headtop");
			var wrapper = document.getElementById("wrapper");
			var pageH = document.body.scrollHeight || document.documentElement.scrollHeight;
			var pageOneH = header.offsetHeight;
			var pageTwo = wrapper.offsetHeight;
			var fh = pageH-pageOneH-pageTwo;
			
			function fixedPosition(){
				var scrollHeight = document.body.scrollTop || document.documentElement.scrollTop;
				if(scrollHeight > 550){
					oZt.style.bottom = fh + "px";
					}
					else{
						oZt.style.bottom = 5 + "px";
						}
				}*/
			//window.setInterval("fixedPosition()",1);
			/*if(screen.width == 1024){
				oZt.style.width = 24 + "px";
				oZt.style.height = 96 + "px";
				oZt.style.backgroundImage = "url(static/image/more.png)";
				oZt.style.left =5 + "px";
				var oA = oZt.getElementsByTagName("a").item(0);
				oA.style.width = 24 + "px";
				oA.style.height = 96 + "px";
				}*/
		</script>
    </div>
</body>
</html>
