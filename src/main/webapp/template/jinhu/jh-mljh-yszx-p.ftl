<!DOCTYPE html>
		...
		<#list channels as chan>
                <div class="col-xs-4">
                    <a href="${chan.url!}" class="mljh-shly-tbg fontSize18 ${(chan_index==0)?string('active','')}">${chan.displayName!}</a>
                </div>
		</#list>
		...
		<#list channels as chan>
		<#if (chan_index==0)>
		<@contentListDirective channelId=chan.id pageSize=channel.pageSize pageNum= pageNum>
