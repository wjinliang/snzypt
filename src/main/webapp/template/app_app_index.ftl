...
<html>
	...
	<body>
		<header class="mui-bar mui-bar-nav">
			<a class="mui-icon mui-icon-home mui-icon-right-nav mui-pull-right" id="homeBtn"></a>
			<!--<a class="mui-icon mui-icon-bake mui-icon-left-nav mui-pull-left" id="backBtn"></a>-->
...
<script src="/html/shenong-app/js/jquery-1.12.1.min.js"></script>
	...
	beforeback: function(){
		plus.webview.all().forEach(function(wv, index){
		    if(index !== 0 && wv.id !== "main_index.htm" && wv.id !== "app_index.htm"){
		...
		return true;
			...
			mui.plusReady(function() {
				...
				var Index = 0;
				//把子页面路径写在数组里面
				var subpages = ['main.htm', 'detail_list.htm', 'detail_shengchan.htm', 'detail_lvyou.htm', 'detail_keji.htm'];
				...
				document.getElementById("homeBtn").addEventListener("tap", function(){
					...
					window.location.href = "main_index.htm";
				})
			})
    </script>
